...
/*jshint esversion: 6 */
/*
* This script is responsible for minification of the build process, including:signing linting, packaging, and generating the copyright documentation with embedded licenses.  
*/
...
`;
  ...
  loadCustomRules(filePath) {
    ...
    try {
      ...
      const customRules = JSON.parse(fileContent);
      
      this.success(`✅ Loaded ${Object.keys(customRules).length} custom providers`);
      
      // Validate custom rules structure
      for (const [providerName, providerData] of Object.entries(customRules)) {
        if (!providerData.urlPattern) {
          throw new Error(`Provider "${providerName}" missing urlPattern`);
        }
        if (!providerData.rules || !Array.isArray(providerData.rules)) {
          throw new Error(`Provider "${providerName}" missing or invalid rules array`);
        }
      }
  ...
  // ENHANCED: Merge providers with same URL pattern (superior logic from second script)
  mergeProvidersByUrlPattern(providers, officialProviderNames) {
    this.info('🔄 Analyzing providers for merge opportunities...');
    
    // Group providers by urlPattern
    const urlPatternGroups = {};
    
    for (const [providerName, providerData] of Object.entries(providers)) {
      const pattern = providerData.urlPattern;
      ...
      urlPatternGroups[pattern].push({
        name: providerName,
        data: providerData,
        isOfficial: officialProviderNames.has(providerName)
      });
    }
    ...
    for (const [urlPattern, providerGroup] of Object.entries(urlPatternGroups)) {
      if (providerGroup.length === 1) {
        // Single provider, keep as is
        const provider = providerGroup[0];
        mergedProviders[provider.name] = provider.data;
      } else {
        // Multiple providers with same urlPattern - merge them
        const mergedProvider = this.mergeProvidersGroup(providerGroup);
        const mergedName = this.createMergedProviderName(providerGroup);
        mergedProviders[mergedName] = mergedProvider;
        
        const domain = this.extractDomainFromPattern(urlPattern) || 'unknown';
        this.info(`  ⚡ Merged ${providerGroup.length} providers for: ${domain}`);
        this.info(`     ${providerGroup.map(p => p.name).join(', ')} → ${mergedName}`);
  ...
  // ENHANCED: Merge providers group (superior logic)
  mergeProvidersGroup(providerGroup) {
    ...
    for (const provider of providerGroup) {
      ...
      // Boolean flags - if ANY provider has it true, keep it true
      if (data.completeProvider) {
        merged.completeProvider = true;
      }
      if (data.forceRedirection) {
  ...
  // ENHANCED: Create merged provider name with official priority
  createMergedProviderName(providerGroup) {
    // Prioritize official provider names when merging
    const officialProviders = providerGroup.filter(p => p.isOfficial);
    
    if (officialProviders.length > 0) {
      const officialName = officialProviders[0].name;
      this.info(`    📌 Using official provider name: ${officialName}`);
      return officialName;
    }
    
    // Use shortest name as it's likely the primary one
    const allNames = providerGroup.map(p => p.name);
    const shortestName = allNames.sort((a, b) => a.length - b.length)[0];
    
    this.info(`    🎯 Using primary provider name: ${shortestName}`);
    return shortestName;
  }

  // ENHANCED: Extract domain from pattern
  extractDomainFromPattern(urlPattern) {
    // Extract domain name from URL pattern regex
    const match = urlPattern.match(/\\\\\\.([a-zA-Z0-9-]+)\\\\\./);
    if (match) return match[1];
    
    const simpleMatch = urlPattern.match(/([a-zA-Z0-9-]+)\\\\\./);
...
this.info(
  ...
  'Terms of Service: https://docs.github.com/en/site-policy/github-terms/github-terms-of-service'
      ...
      const officialProviderNames = new Set(Object.keys(officialRules.providers));
      ...
      const allProviders = {
        ...officialRules.providers,
        ...customRules
      };
      
      const totalBeforeMerge = Object.keys(allProviders).length;
      this.info(`📊 Total before merge: ${totalBeforeMerge} providers`);
      
      // Merge providers with same urlPattern (using enhanced logic)
      const mergedProviders = this.mergeProvidersByUrlPattern(allProviders, officialProviderNames);
...
`;
  ...
  async buildExtension() {
    ...
    this.info('Step 1/5: Generating copyright documentation...');
    await this.generateCopyrightDocumentation();
    
    // 2. Run custom clearurls builder (with enhanced logic)
    this.info('Step 2/5: Building ClearURLs rules...');
    ...
    this.info('Step 5/5: Building extension package...');
...
`;
  ...
  async showMenu() {
    ...
    this.log(`\n${colors.green}Choose an option (0-9, g, t, p, l, v, s, c, q): ${colors.reset}`, 'green');
  ...
  showHelp() {
    ...
    this.log('\nCommands:', 'cyan');
    this.log('  build                 Build extension (copyright + clearurls + fonts + web-ext)', 'white');
    ...
    this.log('  4. Validate project structure', 'dim');
    this.log('  5. Build extension package with web-ext', 'dim');
