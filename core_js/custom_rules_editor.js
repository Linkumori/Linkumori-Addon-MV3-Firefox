 ...
 * 
 * SVG Icons Attribution (JavaScript):
 * ==================================
 * Modified Google Material Icons are embedded in generated HTML (Apache License 2.0).
 * Apache License icon modifications by Subham Mahesh.
 * Initial SVG modification dates:
 * - August 21, 2025
 * - September 5, 2025
 * Subsequent modifications are tracked in git history.
 ...
 * Embedded icon derivatives currently used in this file:
 * 
 ...
 * - Save/Close action icons are no longer embedded by JavaScript in this file.
 *   (Those action button SVGs are defined in customrules.html.)
 * Attribution notice updated: February 23, 2026 by Subham Mahesh.
 * you should refer html notice as well customerules.html to get full picture
 * 
 * //////////
 *  * - No embedded List/Menu icon is currently used. 22 feb 2026
...
async function deleteAllProvidersFromPanel() {
    ...
    try {
        ...
        alert(i18n('customRulesEditor_deleteAllFailed'));
    }
}
...
async function clearAllDisabledRules() {
    ...
    if (!confirm(i18n('providerImport_disabledClearAllConfirm'))) {
        return;
    }
...
async function saveCurrentProvider() {
    ...
    try {
        ...
        if (!jsonEditor || !jsonEditor.value.trim()) {
            alert(i18n('customRulesEditor_jsonRequired'));
            return;
        }
        const provider = JSON.parse(jsonEditor.value);
        ...
        if ((!provider.urlPattern || provider.urlPattern.trim() === '') && 
            ...
            alert(i18n('customRulesEditor_urlPatternOrDomainPatternsRequired'));
            return;
        }
        ...
        if (provider.urlPattern && provider.urlPattern.trim() !== '' && 
            provider.domainPatterns && provider.domainPatterns.length > 0) {
            alert(i18n('customRulesEditor_urlPatternAndDomainPatternsExclusive'));
            return;
        }
        ...
        if (provider.domainPatterns && provider.domainPatterns.length > 0) {
            for (const pattern of provider.domainPatterns) {
                if (!pattern || pattern.trim() === '') {
                    alert(i18n('customRulesEditor_emptyDomainPattern'));
                    return;
                }
                // Basic validation for domain pattern format
                if (!pattern.includes('.') && !pattern.startsWith('||') && !pattern.includes('*')) {
                    alert(i18n('customRulesEditor_invalidDomainPattern', pattern));
                    return;
                }
            }
        }
...
async function handleProviderSubmit(e) {
    ...
    // Validation
    if (!providerName) {
        alert(i18n('customRulesEditor_providerNameRequired'));
        return;
    }
    ...
    if (patternType === 'urlPattern') {
        if (!urlPattern || urlPattern.trim() === '') {
            alert(i18n('customRulesEditor_urlPatternRequired'));
            return;
        }
        ...
        if (domainPatterns.length === 0) {
            alert(i18n('customRulesEditor_domainPatternsRequired'));
            return;
        }
        ...
        alert(i18n('customRulesEditor_patternTypeRequired'));
        return;
    }
...
if (typeof browser !== 'undefined' && browser.storage && browser.storage.onChanged) {
    browser.storage.onChanged.addListener((changes, areaName) => {
        ...
        if (changes[THEME_STORAGE_KEY] && changes[THEME_STORAGE_KEY].newValue) {
            document.documentElement.setAttribute('data-theme', changes[THEME_STORAGE_KEY].newValue);
        }
    });
}
