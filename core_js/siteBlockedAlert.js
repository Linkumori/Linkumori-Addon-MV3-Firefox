...
function initializeTheme() {
    try {
        ...
        // Load theme from browser storage if available
        if (typeof browser !== 'undefined' && browser.storage) {
            browser.storage.local.get(['linkumori-theme']).then(result => {
                if (result['linkumori-theme']) {
                    document.documentElement.setAttribute('data-theme', result['linkumori-theme']);
                    localStorage.setItem('linkumori-theme', result['linkumori-theme']);
                }
            }).catch(error => {
...
function toggleTheme() {
    try {
        ...
        if (typeof browser !== 'undefined' && browser.storage) {
            browser.storage.local.set({ 'linkumori-theme': newTheme }).catch(error => {
