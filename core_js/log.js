...
function normalizeProviderDomainPatterns(providerData) {
    ...
    if (Array.isArray(providerData?.domainPattern)) {
        ...
        patterns.push(providerData.domainPattern.trim());
    }
...
function logMatchDomainPattern(url, patterns) {
    ...
    try {
        ...
        return patterns.some((pattern) => {
            ...
            const wildcardDomainToRegex = (domainPattern) => {
            ...
            const matchRootDomainWildcardTld = (hostnameValue, domainPattern) => {
                const base = domainPattern.slice(0, -2);
            ...
            if (p.startsWith('||')) {
                ...
                if (!domainPattern.includes('/')) {
                    if (domainPattern.includes('*')) {
                        ...
                        if (rootTldOnly) return matchRootDomainWildcardTld(hostname, domainPattern);
                        return new RegExp(wildcardDomainToRegex(domainPattern), 'i').test(hostname);
                    }
                    const normalized = domainPattern.toLowerCase();
...
function getPatternText(context) {
    ...
    } else if (context.patternType === 'domainPattern') {
