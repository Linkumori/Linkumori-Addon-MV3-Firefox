...
${htmlContent}
  ...
  function initializeTheme() {
    try {
      ...
      let savedTheme = localStorage.getItem('linkumori-theme');
      
      // Check browser storage as fallback
      if (!savedTheme && typeof browser !== 'undefined' && browser.storage && browser.storage.local) {
        browser.storage.local.get('linkumori-theme').then(result => {
          if (result['linkumori-theme']) {
            savedTheme = result['linkumori-theme'];
            document.documentElement.setAttribute('data-theme', savedTheme);
            updateThemeButtonAria(savedTheme);
          }
        }).catch(() => {});
      }
      ...
      window.addEventListener('storage', (e) => {
        if (e.key === 'linkumori-theme' && e.newValue) {
          ...
          updateThemeButtonAria(e.newValue);
        }
      });
  ...
  function toggleTheme() {
    try {
      ...
      localStorage.setItem('linkumori-theme', next);
      
      // Save to browser storage
      if (typeof browser !== 'undefined' && browser.storage && browser.storage.local) {
        browser.storage.local.set({ 'linkumori-theme': next }).catch(() => {
        });
      }
